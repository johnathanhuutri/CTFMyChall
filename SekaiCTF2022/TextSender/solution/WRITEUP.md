...
We can divide the solution into 3 stage:

Stage 1: Due to the bug in `edit_message` (compare input string with first part of the receiver name), we can bruteforce the address of heap first.

Stage 2: Due to the bug in `edit_message` (compare input string with first part of the receiver name), we can bruteforce the address of libc.

Stage 3: With heap and libc address, we will conduct a `House of Einherjar` to change `__free_hook` into `system` and free a chunk with string `/bin/sh` to get shell.
